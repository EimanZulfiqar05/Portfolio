      ...
      <div className="max-w-[120rem] mx-auto px-4 sm:px-6 lg:px-12">
        ...
        >
          <motion.span
            ...
            viewport={{ once: true }}
            className="inline-block font-paragraph text-xs sm:text-sm font-bold text-electric-blue uppercase tracking-wider mb-3 md:mb-4"
          >
            What I Do
          </motion.span>
...
function ServiceCard({ icon: Icon, title, tagline, description, features, index }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-gradient-to-br from-foreground/5 to-foreground/[0.02] backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-foreground/10 hover:border-electric-blue/30 transition-all duration-500 overflow-hidden"
    >
      ...
      {/* Icon */}
      <motion.div
        className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-electric-blue/10 border border-electric-blue/20 flex items-center justify-center mb-4 sm:mb-6 group-hover:bg-electric-blue/20 group-hover:border-electric-blue/40 transition-all duration-300"
      ...
      {/* Content */}
      <h3 className="font-heading text-xl sm:text-2xl font-bold text-foreground mb-2 group-hover:text-electric-blue transition-colors duration-300">
        {title}
      </h3>
      
      {tagline && (
        <p className="font-paragraph text-xs sm:text-sm text-soft-purple font-semibold mb-2 sm:mb-3">
          {tagline}
        </p>
      )}

      <p className="font-paragraph text-sm sm:text-base text-foreground/70 mb-4 sm:mb-6 leading-relaxed line-clamp-2">
        {description}
      </p>
      ...
      {features.length > 0 && (
        <ul className="space-y-1 sm:space-y-2">
          {features.slice(0, 2).map((feature, idx) => (
            <li key={idx} className="font-paragraph text-xs sm:text-sm text-foreground/60 flex items-start gap-2">
